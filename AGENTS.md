...
- ../conduit is a framework for ingesting blocks from the Algorand
  blockchain into external applications via a modular plugin pipeline
  (importers, processors, exporters). It reads blocks from an algod
  follower node, so changes to block or transaction structure, or to
  the follower-node sync APIs, may require updates here.
- ../indexer is a REST API providing search over historical on-chain
  data, backed by a PostgreSQL database that Conduit populates. New
  transaction or block fields, and new REST endpoints, often require
  updates here so the data is stored and made queryable.
- ../generator is the shared, template-based code generator
  (`algorand/generator`, a Java/Maven tool driven by Velocity
  templates) that produces the REST client code for the Go, Java, and
  JavaScript SDKs from the algod and indexer OpenAPI specs. Its
  `scripts/generate_{go,java,typescript}.sh` wrappers default to the
  sibling-repo layout used here. Normally a nightly GitHub Action in
  each SDK regenerates and opens a PR; run the generator locally to
  reflect unmerged go-algorand changes. The Python SDK is not
...
- ../go-algorand-sdk contains the Go language SDK to access `algod`
  ...
  signing. New REST endpoints and new transaction or block fields and
  types will often require updates here. ./scripts/export_sdk_types.py
  is useful for preparing changes to accommodate changes to
  go-algorand types.
- ../py-algorand-sdk contains the Python language SDK to access
  `algod` and `indexer`, as well as utilities such as transaction
  creation and signing. New REST endpoints and new transaction or
  block fields and types will often require updates here.
- ../java-algorand-sdk contains the Java language SDK to access
  `algod` and `indexer`, as well as utilities such as transaction
  creation and signing. New REST endpoints and new transaction or
  block fields and types will often require updates here.
- ../js-algorand-sdk contains the JavaScript (moreso TypeScript)
  language SDK to access `algod` and `indexer`, as well as utilities
  such as transaction creation and signing. New REST endpoints and new
  transaction of block fields and types will often require updates
  here.
